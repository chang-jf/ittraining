...
st_am2321 am2321() {
 ...
 retry:
  ...
  ret = write( fd, data, 3 );
  if ( ret < 0 && retry_cnt++ < 50000 ) {
    fprintf( stderr, "am2321: retry\n" );
	lcd_putStr_at(1,3," error    ");
	lcd_putStr_at(2,3," error    ");
    udelay( 9000 );
	sleep(1);
    goto retry;
  }
...
void print_am2321_mackerel_readable( st_am2321 measured ) {
  ...
  printf( "room.%d.temperatre\t%d.%d\t%d\n", ROOM_ID, am2321_temperature_integral( measured ), am2321_temperature_fraction( measured ), (uintmax_t)epoch_time);
  printf( "room.%d.humidity\t%d.%d\t%d\n", ROOM_ID, am2321_humidity_integral( measured ), am2321_humidity_fraction( measured ), (uintmax_t)epoch_time);
  return;
}
