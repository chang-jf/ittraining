...
static void welcome()
{
	...
	lcd_print_WithPosition("Welcome",1,0);
	lcd_print_WithPosition("IT Training",2,0);
	msleep(3000);
...
/*
LCD display as Temperature and Humidity format 
*/

static void lcd_format()
{
	
	lcd_print_WithPosition("T:",1,0);
	lcd_print_WithPosition("C",1,8);
	lcd_print_WithPosition("H:",2,0);
	lcd_print_WithPosition("%",2,8);
...
static int __init klcd_init(void)
{
	struct device *dev_ret;
        gpio_direction_output(LCD_RW_PIN_NUMBER,0);         
	...
	welcome();
	lcd_initialize();
	//set LCD display as Temperature and Humidity format 
	lcd_format();
